...
export const get_todos = () => {
  return document.querySelectorAll("[data-todo]");
};

export function complete_todo({elem, init = false}) {
  const retVal = {
    fn(evt) {
      elem = elem ?? evt.target;
      if (
        elem.classList.contains("complete-btn") === false
      ) {
        return;
      }
  
      const todo = JSON.parse(elem.closest('article').dataset.todo);
    ...
    init() {
      elem.addEventListener("click", retVal.fn);
    },
    remove() {
      elem.removeEventListener("click", retVal.fn);
    }
  }
  if (init) {
    retVal.init()
...
export function todo_update_cb({elem, init = false}) {
  const retVal = {
    fn(evt) {
      ...
      if (todoFound === null) {
        fetch(`${Sealed.ORIGIN}/update/${todo.todo_id}`)
          ...
          .then((html) => {
            ...
            description_resize(content.querySelector('textarea[name="description"]'));
...
export function delete_todo({elem, init = false}) {
  const retVal = {
    fn(evt) {
      elem = elem ?? evt.target;
      if (
...
export const description_resize = (description) => {
